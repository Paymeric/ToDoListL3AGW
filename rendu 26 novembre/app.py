    ...
    if request.method == 'POST':
        ...
        if conn is not None:
            ...
            if user:
                ...
                # Lancer l'interface graphique dans un thread secondaire (pas un thread Flask, mais un thread séparé)
                threading.Thread(target=todo.run_todo_app, args=(user['id'],), daemon=True).start()
...
def run_flask():
    app.run(debug=True, use_reloader=False, threaded=True)  # Utiliser threaded=True pour Flask en mode non-bloquant

if __name__ == '__main__':
    # Lancer l'application Flask sans thread
